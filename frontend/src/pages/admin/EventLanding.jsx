...
export default function EventLandingPage() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        ...
        console.error("Failed to fetch event landing page:", error);
        setPage(null);
      } finally {
        setLoading(false);
      }
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-white via-indigo-50/20 to-white">
        <div className="mx-auto max-w-6xl px-4 py-20 md:px-6">
          <div className="rounded-3xl border border-indigo-100 bg-white/80 p-8 backdrop-blur-sm shadow-lg">
            ...
            <div className="mt-2 h-5 w-5/6 animate-pulse rounded bg-indigo-50"></div>
          </div>
        </div>
      </section>
    );
        ...
        <div className="relative mx-auto max-w-4xl px-4 py-20 text-center md:px-6">
          <div className="rounded-3xl border border-indigo-100 bg-white/80 p-10 backdrop-blur-sm shadow-lg">
            ...
            <p className="mt-4 text-indigo-900/70">
              The event you are looking for is unavailable or has not been published.
            </p>
            <Link
              to="/"
  ...
  const eyebrow = page?.hero?.eyebrow || "Event";
  const headline = page?.hero?.headline || page?.title;
  const subheadline = page?.hero?.subheadline || "";
  const ctaText =
    page?.hero?.ctaText ||
    (page?.eventDetails?.price > 0 ? "Reserve Your Spot" : "Register Now");
  const ctaLink =
    page?.hero?.ctaLink || page?.eventDetails?.registrationLink || "/contact";

  const eventDate = page?.eventDetails?.date
    ? new Date(page.eventDetails.date).toLocaleDateString('en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric'
      })
    : "";
  const eventTime = page?.eventDetails?.time || "";
  const eventLocation = page?.eventDetails?.location || "";
  const eventPrice = page?.eventDetails?.price || 0;
  const eventCapacity = page?.eventDetails?.capacity || 0;
      ...
      <section className="relative overflow-hidden min-h-[80vh] flex items-center">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 md:px-6 lg:py-20">
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:max-w-2xl">
                ...
                {eventTime && (
                  <div className="group relative overflow-hidden rounded-2xl border border-white/20 bg-white/10 backdrop-blur-sm p-4 transition-all hover:border-[#FFD700]">
                    ...
                    <p className="mt-2 text-base font-semibold text-white">{eventTime}</p>
                  </div>
                )}
                ...
                {eventLocation && (
                  <div className="group relative overflow-hidden rounded-2xl border border-white/20 bg-white/10 backdrop-blur-sm p-4 transition-all hover:border-[#FFD700]">
                    ...
                    <p className="mt-2 text-base font-semibold text-white">{eventLocation}</p>
                  </div>
                )}
                ...
                <div className="group relative overflow-hidden rounded-2xl border border-white/20 bg-white/10 backdrop-blur-sm p-4 transition-all hover:border-[#FFD700]">
                  ...
                  <p className="mt-2 text-base font-semibold text-white">
                    {eventPrice > 0 ? `KES ${eventPrice.toLocaleString()}` : "Free"}
                  </p>
                </div>
              </div>
            ...
            {/* Right Image (if no hero image, show placeholder) */}
            {!heroImage && (
            ...
            <div>
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html:
                    page.description ||
                    "<p>Event details will be shared here soon.</p>",
                }}
              />
            </div>
              ...
              <div className="relative rounded-3xl border border-indigo-100 bg-white/80 p-6 backdrop-blur-sm shadow-lg">
                ...
                <div className="mt-6 space-y-4">
                  ...
                  {eventTime && (
                    <div className="rounded-2xl border border-indigo-100 bg-white p-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#FFD700]">Time</p>
                      <p className="mt-2 text-sm font-medium text-black">{eventTime}</p>
                    </div>
                  )}

                  {eventLocation && (
                    <div className="rounded-2xl border border-indigo-100 bg-white p-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#FFD700]">Venue</p>
                      <p className="mt-2 text-sm font-medium text-black">{eventLocation}</p>
                    </div>
                  )}
                  ...
                  {eventCapacity > 0 && (
                    <div className="rounded-2xl border border-indigo-100 bg-white p-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#FFD700]">Capacity</p>
                      <p className="mt-2 text-sm font-medium text-black">
                        {eventCapacity} attendees
                      </p>
                    </div>
                  )}
                </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-indigo-900 to-black">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 text-center md:px-6 lg:py-24">
          ...
          <h2 className="font-serif text-3xl font-bold leading-tight text-white md:text-4xl lg:text-5xl">
            If this event aligns with your goals, this is the time to act.
          </h2>

          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-indigo-200">
            Reserve your place and take the next step toward stronger strategy,
            better systems, and clearer execution.
          </p>
